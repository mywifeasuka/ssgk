...
int do_wait(int pid) {
  int have_kids = 0;
  for (int i = 0; i < NPROC; i++) {
    if (procs[i].status == FREE) continue;
      ...
      case CODE_SEGMENT:
        {
            ...
            for (int j = 0; j < npages; j++) {
                uint64 current_va = code_va + j * PGSIZE;
                uint64 pa = lookup_pa(parent->pagetable, current_va);
                user_vm_map((pagetable_t)child->pagetable, current_va, PGSIZE, pa,
